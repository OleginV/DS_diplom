...
-- температура
	...
	sum(coalesce(w.t_temperature,0)) as sum_air_temp,
	min(w.tn_min_temperature) as min_temp,
	sum(coalesce(w.tn_min_temperature,0)) as sum_min_temperature,
	max(w.tx_max_temperature) as max_temp,
	sum(coalesce(w.tx_max_temperature,0)) as sum_max_temperature,
	...
	sum(coalesce(w.td_dew_point_temp,0)) as sum_dew_point_temp,
	...
	sum(coalesce(w.tg_min_ground_temp,0)) as sum_min_ground_temp,
-- давление
...
-- ветер
	...
	sum(coalesce(w.ff_wind_speed,0)) as sum_wind_speed,
...
-- осадки
	sum(coalesce(w.rrr_precipitation::numeric,0)) as sum_precipitation,
	avg(w.rrr_precipitation::numeric) as avg_precipitation,
	sum(coalesce(w.tr_precipitation_time::numeric,0)) as sum_precipitation_time,
	avg(w.tr_precipitation_time) as avg_precipitation_time,
	coalesce(sum(coalesce(w.rrr_precipitation::numeric,0)) / nullif(sum(coalesce(w.tr_precipitation_time::numeric,0)),0), 0) as precipitation_speed,
-- состояние почвы
	avg(case 
